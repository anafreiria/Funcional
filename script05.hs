...
f1 [a] = [a]
f1 (a:b:x)
    | isDigit a && not (isDigit b) = a : (replica(charToInt a) b)  ++ f1 x
...
num a
    ...
    |otherwise = error "erro caracter nao é um digito"
...
strToInt :: String -> Int
strToInt [] = 0
strToInt (x:xs) = strToInt xs * 10 + digitToInt x
...
f11 [a]    = (a, False, 1):[]
f11 (a:b:x)
    | isDigit a && not (isDigit b) = (a, False, 1):(b, True, (charToInt a)):f11 x
    | otherwise = (a, False, 1):f11 (b:x)
...
alfToChar :: Int -> Char
alfToChar x = chr (ord 'a' + x - 1)
--ord 'a' = 97

avalia [] = False
avalia (a:x) = isDigit a || avalia x
